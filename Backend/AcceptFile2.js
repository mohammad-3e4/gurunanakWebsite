...
// Serve uploaded files
app.use('/uploads/certificate', express.static(join(__dirname, 'uploads', 'certificate')));
...
app.post('/uploadcertificate', upload5.single('file'), async (req, res) => {
    try {
        ...
        if (uploadedFile) {
            await con.execute(`INSERT INTO  certificate (file_name, description, date) VALUES (?, ?, ?)`, [uploadedFile.filename, description, date]);
            console.log("File saved to database.");
            return res.status(200).send('File uploaded and saved to database.');
        } else {
            await con.execute(`INSERT INTO pdf_files (description, date) VALUES (?, ?)`, [description, date]);
            console.log("Description and Date saved to database.");
            return res.status(200).send('No file uploaded, but description and date saved to database.');
        }
    } catch (error) {
        console.error(error);
        return res.status(500).send('Error saving file to database.');
...
app.get('/certificate', async (req, res) => {
    try {
        ...
        const [rows, fields] = await con.execute("SELECT *, DATE_FORMAT(date, '%m/%d/%Y') AS date FROM certificate");
...
app.post('/uploadfile', upload6.single('file'), async (req, res) => {
    try {
        ...
        // Insert into the database, handle file and no file cases separately
        if (uploadedFile) {
            await con.execute(`INSERT INTO news (name, description, date) VALUES (?, ?, ?)`, [uploadedFile.filename, description, date]);
            console.log("File saved to database.");
            return res.status(200).send('File uploaded and saved to database.');
        } else {
            await con.execute(`INSERT INTO news (description, date) VALUES (?, ?)`, [description, date]);
            console.log("Description and Date saved to database.");
            return res.status(200).send('No file uploaded, but description and date saved to database.');
        }
    } catch (error) {
...
app.get('/files', async (req, res) => {
    try {
        // const [rows, fields] = await con.execute('SELECT * FROM pdf_files');
        const [rows, fields] = await con.execute("SELECT *, DATE_FORMAT(date, '%m/%d/%Y') AS date FROM news");
...
app.post('/uploadimage', upload2.single('file'), async (req, res) => {
    try {
        ...
        await con.execute(`INSERT INTO images (Name) VALUES (?)`, [uploadedFile.filename]);
...
app.get('/:activity', async (req, res) => {
    ...
    try {
        const [rows, fields] = await con.execute(`SELECT * FROM activity WHERE activity = "${activity}";`);
        res.json(rows);
    } catch (error) {
        console.error(error);
        res.status(500).send('Error fetching files from database.');
...
// Serve uploaded files
app.use('/uploads/result', express.static(join(__dirname, 'uploads', 'result')));
// POST endpoint to upload a file with optional description and date
app.post('/uploadresult', upload4.single('file'), async (req, res) => {
    try {
        ...
        // Insert into the database, handle file and no file cases separately
        if (uploadedFile) {
            await con.execute(`INSERT INTO result (file_name, description, date) VALUES (?, ?, ?)`, [uploadedFile.filename, description, date]);
            console.log("File saved to database.");
            return res.status(200).send('File uploaded and saved to database.');
        } else {
            await con.execute(`INSERT INTO pdf_files (description, date) VALUES (?, ?)`, [description, date]);
...
app.get('/api/result', async (req, res) => {
    try {
        const [rows, fields] = await con.execute("SELECT *, DATE_FORMAT(date, '%m/%d/%Y') AS date FROM result");
        res.json(rows);
        // console.log(rows);
    } catch (error) {
        console.error(error);
...
// Serve uploaded files
app.use('/uploads/admissionform', express.static(join(__dirname, 'uploads', 'admissionform')));
// POST endpoint to upload a file with optional description and date
app.post('/uploadform', upload7.single('file'), async (req, res) => {
    try {
        ...
        // Insert into the database, handle file and no file cases separately
        if (uploadedFile) {
            await con.execute(`INSERT INTO admissionform (file_name, description, date) VALUES (?, ?, ?)`, [uploadedFile.filename, description, date]);
            console.log("File saved to database.");
            return res.status(200).send('File uploaded and saved to database.');
        } else {
            await con.execute(`INSERT INTO admissionform (description, date) VALUES (?, ?)`, [description, date]);
            console.log("Description and Date saved to database.");
            return res.status(200).send('No file uploaded, but description and date saved to database.');
        }
    } catch (error) {
...
app.get('/api/form', async (req, res) => {
    try {
        const [rows, fields] = await con.execute("SELECT *, DATE_FORMAT(date, '%m/%d/%Y') AS date FROM admissionform");
        res.json(rows);
        // console.log(rows);
    } catch (error) {
        console.error(error);
...
app.post('/uploadcarousel', upload7.single('file'), async (req, res) => {
    try {
        ...
        const [existingSlide] = await con.execute(`SELECT * FROM carousel WHERE slide = ?`, [slide]);

        if (existingSlide.length > 0) {
            // If slide exists, update the file_name for that slide
            await con.execute(`UPDATE carousel SET file_name = ? WHERE slide = ?`, [uploadedFile.filename, slide]);
            console.log("File name updated in database.");
            return res.status(200).send('File uploaded and file name updated in database.');
        } else {
            // If slide does not exist, insert a new record
            await con.execute(`INSERT INTO carousel (slide, file_name) VALUES (?, ?)`, [slide, uploadedFile.filename]);
            console.log("File saved to database.");
            return res.status(200).send('File uploaded and saved to database.');
        }
    } catch (error) {
...
app.post('/api/salary', async (req, res) => {
    ...
    const { name, designation, qualification, experience, pay_scale, category } = req.body;
...
app.post('/api/joining', async (req, res) => {
    ...
    const { name, designation, qualification, date_of_birth, date_of_appointment, date_of_retirement, category } = req.body;
    ...
    try {
        const result = await con.query(
            ...
            [name, designation, qualification, date_of_birth, date_of_appointment, date_of_retirement, category]
        );
        console.log("Data inserted successfully");
        res.send("Data inserted successfully");
    } catch (error) {
...
app.get('/api/joining', async (req, res) => {
    try {
        const [rows, fields] = await con.execute("SELECT *, DATE_FORMAT(date_of_birth, '%m/%d/%Y') AS date_of_birth, DATE_FORMAT(date_of_appointment, '%m/%d/%Y') AS date_of_appointment,DATE_FORMAT(date_of_retirement, '%m/%d/%Y') AS date_of_retirement FROM faculty_joining");
        res.json(rows);
        // console.log(rows);
    } catch (error) {
        console.error(error);
