...
function AdminGallery() {
    ...
    const handleDelete = async (img_name) => {
        try {
            await axios.delete(`http://localhost:5000/images/${img_name}`);
            //  console.log(`${news_id}`);
        }
        catch (err) {
            console.log(err);
                    ...
                    <div className='grid relative grid-cols-2  w-4/5 md:grid-cols-3 gap-4'>
                        {files.map((file, index) => (
                            <div className=' relative w-full rounded-lg bg-orange ' key={index}>
                                <img className="h-full w-full duration-200 rounded-lg hover:skew-y-3 " src={`http://localhost:5000/uploads/images/${file.Name}`} alt={`${file.Name} is not available`} />
