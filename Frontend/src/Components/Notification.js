...
//                     {data.map((item, index) => (
//                         <li
//                             className={`flex justify-between p-3 border-b items-center ${item.clicked ? 'font-normal' : 'font-bold'}`}
//                             key={index}
...
export default function Notification() {
    ...
    const handleItemClick = async (index) => {
        ...
        if (clickedItem.status === 1) {
            ...
            try {
                const response = await axios.put(`http://localhost:5000/api/notification/${clickedItem.notification_id}`, {
                    status: 0,
                });
